...
def test_consistency():
    ...
    for _ in range(10):
        ...
        velocities = extension_state.getVelocities()
        kineng = context._extra_kinetic_energy  # pylint: disable=protected-access
        for i, xdof in enumerate(context.getExtraDOFs()):
            _, (_, velocity) = kineng.getParticleParameters(i)
            assert velocities[i].x == pytest.approx(velocity)
            assert velocities[i].y == pytest.approx(0)
            assert velocities[i].z == pytest.approx(0)
